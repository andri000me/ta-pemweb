					...
					<div class="panel-body">
						...
						<table data-toggle="table" data-url="#"  data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search="true" data-select-item-name="toolbar1" data-pagination="true" data-sort-name="name" data-sort-order="desc" id="tebel">
						    ...
						    <?php foreach ($keca as $k) {
						    	$id_kec = $k->kode_kec;
						    	$sd = $this->my_lib->jum_kepsek(array('kec'=>$id_kec,'jenjang'=>'SD'));
						    	$mi = $this->my_lib->jum_kepsek(array('kec'=>$id_kec,'jenjang'=>'MI'));
						    	$smp = $this->my_lib->jum_kepsek(array('kec'=>$id_kec,'jenjang'=>'SMP'));
						    	$mts = $this->my_lib->jum_kepsek(array('kec'=>$id_kec,'jenjang'=>'MTS'));
						    	$sma = $this->my_lib->jum_kepsek(array('kec'=>$id_kec,'jenjang'=>'SMA'));
						    	$ma = $this->my_lib->jum_kepsek(array('kec'=>$id_kec,'jenjang'=>'MA'));
						    	$smk = $this->my_lib->jum_kepsek(array('kec'=>$id_kec,'jenjang'=>'SMK'));
						    	$sdmi = $sd + $mi;
						    	$smpmts = $smp + $mts;
						    	$smama = $sma + $ma;
