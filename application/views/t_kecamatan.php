      				...
      				<table id="dataTables" class="table table-hover table-striped dataTable DTTT_selectable" role="grid">
      					<thead>
      						<tr role="row">
      							...
      							<th class="text-right sorting" tabindex="0" aria-controls="dataTables" rowspan="1" colspan="1" aria-label="Guru: activate to sort column ascending">Pegawai</th>
      						</tr>
      					</thead>
      					<tbody>
      					<?php
						...
						$total = array(0,0,0);
      					foreach ($sekolah as $key) {
      						...
      						'<tr class="data odd" role="row">
      							<td>'.$no++.'</td>
      							<td><a href="'.base_url().'index.php/data_pegawai/pegawai/'.$key['npsn'].'">'.$key['nama_sekolah'].'</a></td>
      							...
      							<td style="width: 10%;" class="text-right">'.$key['jumlah_pegawai'].'</td>
      							
      						</tr>';
							$total[0] = $total[0]+$key['jumlah_siswa'];
							$total[1] = $total[1]+$key['rombel'];
							$total[2] = $total[2]+$key['jumlah_pegawai'];
      					}
      					?>
      					</tbody>
      					<tfoot>
